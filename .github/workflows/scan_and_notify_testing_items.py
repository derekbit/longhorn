...
def flatten_issues(title, blocks, issues, user_mappings):
    ...
    for issue in issues:
        ...
        for assignee in issue["content"]["assignees"]["nodes"]:
            slack_id = user_mappings.get(assignee["login"])
            if not slack_id:
                assignees.append(assignee["login"])
            else:
                assignees.append("<@" + slack_id + ">")
        ...
        blocks.append({
            ...
            "text": {
                "type": "mrkdwn",
                "text": f"*<{issue_url}|{number}>* - {title} - {', '.join(assignees)}"
            }
        })
...
def send_slack_notification(webhook_url, user_mappings,
    ...
    blocks = flatten_issues("Current Issues",
                            blocks, current_issues, user_mappings)
    blocks = flatten_issues("Non-Current Issues",
                            blocks, non_current_issues, user_mappings)
...
def scan_and_notify(github_org, github_repo, github_project):
    ...
    send_slack_notification(webhook_url,
                            user_mappings, current_issues, non_current_issues)
